...
import uo.ri.ui.util.ContractBuilder;
import uo.ri.ui.util.MechanicBuilder;
import uo.ri.ui.util.PayrollBuilder;
...
public class ContractTests {
	...
	public void setUp() throws Exception {
		...
		contract = new ContractBuilder().forMechanic(mechanic)
				.withStartDate(START_DATE).withBaseSalary(BASE_SALARY).build();
	}
	...
	public void testEndDateGreaterThanStartDateLesser() {
		/*
		 * The contract start date is always the 1st day of the month and the
		 * end date is always the last day of the month passed to
		 * marksAsFinished(...), then, with this endDate, the end date will be
		 * the last day of the previous month, which is an error.
		 */
	...
	/**
	 * The end date passed may be the start date, but the contract's end date is
	 * set to the last date of the month anyway
	 */
	@Test
	...
	public void testWith12MonthsCompensation() {
		...
		/*
		 * One year gross salary is just the year base salary because - there
		 * are no trienniums - there is no productivity plus for the payrolls
		 * generated
		 */
		double expected = BASE_SALARY / 365
	...
	public void testMostRecentPayroll() {
		...
		new PayrollBuilder() // automatically linked with contract (constructor)
				.forContract(contract).withDate(firstPayrollDate).build();

		Payroll lastPayroll = new PayrollBuilder().forContract(contract)
				.withDate(secondPayrollDate).build();

		assertTrue(contract.getLastPayroll() == lastPayroll);
	}

	/**
	 * An already finished contract raises exception trying to mark it as
	 * finished again
	 */
	...
	/**
	 * startDate is copied in the constructor even if it already is the first
	 * day of month
	 */
	...
	/**
	 * endDate is copied in the constructor even if it already is the last day
	 * of month
	 */
	@SuppressWarnings("deprecation") // for setDate
	...
	/**
	 * endDate is copied in the setter even if it already is the last day of
	 * month
	 */
	@SuppressWarnings("deprecation") // for setDate
