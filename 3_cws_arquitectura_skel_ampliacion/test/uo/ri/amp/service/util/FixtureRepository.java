...
public class FixtureRepository {
	...
	public static Metalico findCashByClientId(Long id) {
		return repository().forMedioPago().findByClientId(id).stream()
				.filter(mp -> mp instanceof Metalico).map(mp -> (Metalico) mp)
	...
	public static Cliente registerNewClient() throws BusinessException {
	...
	public static Cliente registerNewClientWithCash() throws BusinessException {
		Cliente c = registerNewClient();
	...
	public static Vehiculo registerNewVehicle() throws BusinessException {
	...
	public static TarjetaCredito registerNewCreditCard()
		...
		TarjetaCredito tc = new TarjetaCredito(number, "TC",
				DateUtil.tomorrow());
	...
	public static Factura registerNewInvoiceForClientWithAmount(Cliente c,
			double amount) throws BusinessException {
		Factura f = createInvoiceForAmount(amount);
		Vehiculo v = f.getAverias().stream().findFirst().orElse(null)
				.getVehiculo();
		Association.Poseer.link(c, v);
		repository().forFactura().add(f);
		return f;
	...
	public static Factura registerNewSettledInvoiceForAmount(Cliente c,
			double amount) throws BusinessException {
	...
	private static Averia newAveria(Vehiculo v) throws BusinessException {
		sleep(5 /* ms */ );
	...
	public static Recomendacion registerNewClientWithBreakdownRecommendedBy(
			Cliente recommender) throws BusinessException {
	...
	public static Recomendacion registerNewClientRecommendedBy(
			Cliente recommender) throws BusinessException {
		Cliente c = registerNewClient();
		Recomendacion r = new Recomendacion(recommender, c);
		repository().forRecomendacion().add(r);
